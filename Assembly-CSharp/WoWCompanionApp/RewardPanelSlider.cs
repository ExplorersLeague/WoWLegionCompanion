...
namespace WoWCompanionApp
{
	public class RewardPanelSlider : MonoBehaviour
	{
		...
		public void OnAddMissionLootToRewardPanel(int garrMissionID)
		{
			...
			MissionRewardDisplay.InitMissionRewards(AdventureMapPanel.instance.m_missionRewardResultsDisplayPrefab, this.m_rewardIconArea.transform, wrapperGarrisonMission.Rewards);
			if (wrapperGarrisonMission.MissionState != 3)
			{
				return;
			}
			...
			if (wrapperGarrisonMission.OvermaxRewards.Count > 0)
			{
				...
				component.InitReward(MissionRewardDisplay.RewardType.item, wrapperGarrisonMission.OvermaxRewards[0].ItemID, (int)wrapperGarrisonMission.OvermaxRewards[0].ItemQuantity, 0, wrapperGarrisonMission.OvermaxRewards[0].ItemFileDataID);
			}
		}
