...
namespace Newtonsoft.Json.Serialization
{
	internal class JsonSerializerInternalReader : JsonSerializerInternalBase
	{
			...
			if (reader.TokenType == JsonToken.PropertyName)
			{
				...
				for (;;)
				{
					...
					if (string.Equals(a, "$ref", StringComparison.Ordinal))
					{
						...
						text2 = ((reader.Value == null) ? null : reader.Value.ToString());
						this.CheckedRead(reader);
						if (text2 != null)
						{
							goto Block_5;
					...
					else if (string.Equals(a, "$type", StringComparison.Ordinal))
					{
						...
						if (((typeNameHandling == null) ? base.Serializer.TypeNameHandling : typeNameHandling.Value) != TypeNameHandling.None)
						{
							string typeName;
							string assemblyName;
				...
				if (stack.Count == arrayRank)
				{
					...
					if (tokenType != JsonToken.EndArray)
					{
						if (tokenType != JsonToken.Comment)
						{
							...
							{
								if (!base.IsErrorHandled(list, contract, list2.Count, ex))
								{
									throw;
								}
								this.HandleError(reader, depth);
		...
		private void SetPropertyPresence(JsonReader reader, JsonProperty property, Dictionary<JsonProperty, JsonSerializerInternalReader.PropertyPresence> requiredProperties)
		{
			if (property != null)
			{
				requiredProperties[property] = ((reader.TokenType != JsonToken.Null && reader.TokenType != JsonToken.Undefined) ? JsonSerializerInternalReader.PropertyPresence.Value : JsonSerializerInternalReader.PropertyPresence.Null);
			}
		}
