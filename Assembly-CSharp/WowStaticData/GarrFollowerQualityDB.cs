	...
	public class GarrFollowerQualityDB
	{
		...
		public void EnumRecords(Predicate<GarrFollowerQualityRec> callback)
		{
			...
			try
			{
				while (enumerator.MoveNext())
				{
					...
					GarrFollowerQualityRec obj2 = (GarrFollowerQualityRec)obj;
					if (!callback(obj2))
					{
						break;
					}
		...
		public void EnumRecordsByParentID(int parentID, Predicate<GarrFollowerQualityRec> callback)
		{
			...
			try
			{
				...
				{
					object obj = enumerator.Current;
					GarrFollowerQualityRec garrFollowerQualityRec = (GarrFollowerQualityRec)obj;
					if ((ulong)garrFollowerQualityRec.Quality == (ulong)((long)parentID) && !callback(garrFollowerQualityRec))
					{
						break;
					}
				}
